...
void Analyzer::open(std::string filename)
{
    ...
    if(file.is_open()){
        ...
        cerr << "Open:" << filename.c_str() << " failed" << endl;
    }
...
void Analyzer::saveResult(string filename)
{
...
void Analyzer::saveSymbol(string filename)
{
    ofstream file(filename.c_str());
    if(file.is_open()){
        for(auto it: idTable){
...
void Analyzer::saveConstant(string filename)
{
    ofstream file(filename.c_str());
    if(file.is_open()){
